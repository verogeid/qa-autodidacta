...
function loadSelector() {
    fetch(baseUrl + 'speechs.md')
        .then(r => {
        console.log('Fetched speechs.md:', r.ok);
        return r.text();
        })
        .then(text => {
        console.log('Content of speechs.md:\n', text);
        const lines = text.split(/\n+/).filter(l => l.trim());
        selector.innerHTML = '';
        lines.forEach(line => {
        ...
        console.log('Options added to selector:', selector.options.length);
        loadAndPlay(selector.value);
    })
...
export function initAudiobook(title, url) {
    ...
    speechSynthesis.onvoiceschanged = () => {
        ...
        console.log('Initializing audiobook with baseUrl:', baseUrl);
        loadSelector();
    };
